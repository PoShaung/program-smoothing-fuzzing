...
class FuzzNet(nn.Module):
    '''Bi-LSTM Model'''
    def __init__(self, in_dim, num_layers, hidden_1, hidden_2, out_dim):
        ...
        self.fc_1 = nn.Linear(hidden_1*2, hidden_2)
        self.relu = nn.ReLU(inplace=True)
        self.fc_2 = nn.Linear(hidden_2, out_dim)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        out, _ = self.lstm(x.view(len(x), 1, -1)) # input: [batch_size, n_step, n_class]
        out = self.fc_1(out.view(len(x), -1))
        out = self.relu(out)
        out = self.fc_2(out)
...
def construct_bitmap(seed_list, program_execute):
    ...
    for idx, seed in enumerate(seed_list):
        ...
        for edge in bitmap_ec[seed]:
            bitmap[idx][label_dict[edge]] = 1

    # label reduction
    bitmap = np.unique(bitmap, axis=1)
    all_cover_idx = np.where(np.sum(bitmap == 1, axis=0) == bitmap.shape[0])[0]
    bitmap = np.delete(bitmap, all_cover_idx, 1)
...
def gen_adv(fuzzNet, fuzzData, edge_idx, seed_idx):
    ...
    y, _ = fuzzNet.lstm(x.view(1, 1, -1))
    y = fuzzNet.fc_1(y.view(-1))
    y = fuzzNet.relu(y)
    y = fuzzNet.fc_2(y)
...
def nn_lop(grads_num):
    ...
    fuzzNet = FuzzNet(fuzzData.seed_size, NUM_LAYER, HIDDEN_1, HIDDEN_2, fuzzData.edge_size).to(device)
