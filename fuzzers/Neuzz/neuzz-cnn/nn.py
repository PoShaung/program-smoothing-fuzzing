...
class FuzzNet(nn.Module):
    ...
    def __init__(self, in_dim, hidden, out_dim):
        ...
        self.cnnv1    =  nn.Conv1d(in_channels=in_dim, out_channels=hidden, kernel_size=1)
    ...
    def forward(self, x):
        out = self.cnnv1(x.view(len(x), -1, 1))
        out = self.relu(out)
...
def gen_adv(fuzzNet, fuzzData, edge_idx, seed_idx):
    ...
    y = fuzzNet.cnnv1(x.view(1, -1, 1))
    y = fuzzNet.relu(y)
    y = fuzzNet.max_pool(y)
    y = fuzzNet.fc(torch.squeeze(y))
...
def nn_lop(grads_num):
    ...
    fuzzNet = FuzzNet(fuzzData.seed_size, HIDDEN, fuzzData.edge_size).to(device)
