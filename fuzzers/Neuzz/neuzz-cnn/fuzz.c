...
static u8 run_target(int timeout)
{
  ...
  if (child_pid <= 0)
    perror("Fork server is misbehaving (OOM?)");
...
void fuzz_lop(char *grad_file, int sock)
{
  ...
  while ((nread = getline(&line, &llen, stream)) != -1)
  {
    ...
    if ((line_cnt % 10) == 0)
    {
      printf("###### fuzz %s line_cnt %d edge num %d\n", fn, line_cnt,
  ...
  printf("End of one Fuzzing loop\n");
  free(line);
  fclose(stream);
}
...
void start_fuzz(int f_len)
{
  ...
  while (1)
  {
    ...
    printf("Start of the fuzzing module\n");
...
void main(int argc, char *argv[])
{
  ...
  while ((opt = getopt(argc, argv, "+i:o:l:")) > 0)
    ...
    case 'i': /* input dir */
      ...
      if (in_dir)
        perror("Multiple -i options not supported");
      in_dir = optarg;
      break;
    ...
    case 'o': /* output dir */
      ...
      if (out_dir)
        perror("Multiple -o options not supported");
      out_dir = optarg;
      break;
