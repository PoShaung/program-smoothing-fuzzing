...
def random_process(targert_adv_list):
    random.shuffle(targert_adv_list)
    return targert_adv_list

def reverse_process(targert_adv_list):
    targert_adv_list.reverse()
    return targert_adv_list

def process_adv_list(adv_list, processor):
    res_list = list()
    for tmp_tuple in adv_list:
        # split
        idx = tmp_tuple[0]
        sign = tmp_tuple[1]
        new_list = list()
        for i in range(len(idx)):
            new_list.append((idx[i], sign[i]))
        # process
        new_list = processor(new_list)
        # merge
        idx_res = list()
        sign_res = list()
        for i, s in new_list:
            idx_res.append(i)
            sign_res.append(s)
        res_list.append((idx_res, sign_res, tmp_tuple[2]))
    return res_list
...
def gen_mutate2(model, edge_num, sign):
    ...
    with open('gradient_info_p', 'w') as f:
        for idxx in range(len(interested_indice[:])):
            ...
            adv_list = process_adv_list(adv_list, reverse_process)
